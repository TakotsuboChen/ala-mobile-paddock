...
> ⚠️ **状态（2026-09-12 起）：本文为历史快照，其「差距清单/方案」已全部落地或改写。**
> 当前真实契约见 `../../ala-mobile-tool/docs/PADDOCK_PLAN.md`「Bot 消息规则引擎」节，
> 唯一事实源是 `qq_bot::action_metas()`。§六 差距清单与 §七 方案中的字段清单（如
> `no_code_template`）为当时的提议，与现行实现不同——**不要照本文新增字段**。
> 已落地：单聊并入规则引擎、每类失败独立文案、动作编辑器去镜像化、普通回复身份依赖兜底。
> 新增：`query_username` 动作。未改：§七-C「执行链不数据化」边界（仍成立）。
